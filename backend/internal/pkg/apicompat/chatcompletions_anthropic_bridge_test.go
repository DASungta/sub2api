...
func TestAnthropicToChatCompletionsRequest_MaxTokensFloor(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, out.MaxTokens)
	require.Equal(t, minMaxOutputTokens, *out.MaxTokens)
}
...
func TestDirectBridge_RequestMatchesDoubleConversion(t *testing.T) {
	...
	require.Equal(t, double.MaxTokens, direct.MaxTokens)
...
func TestAnthropicToChatCompletionsRequest_UserArrayContentPreservesParts(t *testing.T) {
	// This fork's Responses-to-Chat conversion preserves array-form content, so
	// the direct bridge must keep the same parts representation.
	req := &AnthropicRequest{
		Model:     "deepseek-v4-pro",
		MaxTokens: 100,
		Messages: []AnthropicMessage{
...
func TestDirectBridge_RequestMatchesDoubleConversion_ArrayUserContent(t *testing.T) {
	// Array-form user content must match the double-conversion chain exactly.
	req := &AnthropicRequest{
		Model:     "deepseek-v4-pro",
		MaxTokens: 100,
		Messages: []AnthropicMessage{
