...
func ResponsesToChatCompletionsRequest(req *ResponsesRequest) (*ChatCompletionsRequest, error) {
	...
	if len(effectiveTools) > 0 {
		...
		out.Tools = tools
		if len(effectiveTools) > len(req.Tools) {
			// additional_tools are routing metadata, not conversation messages.
			out.Messages, err = responsesInputToChatMessages(req.Instructions, req.Input)
			if err != nil {
				return nil, err
			}
		}
	} else if len(req.Tools) > 0 {
		// All declared tools were server-side-only and therefore dropped.
		out.Tools = nil
		out.ToolChoice = nil
	}
	...
	if len(req.ToolChoice) > 0 && len(out.Tools) > 0 {
	...
	} else if len(req.ToolChoice) > 0 && (len(req.Tools) > 0 || len(effectiveTools) > 0) {
		out.ToolChoice = nil
	}
	if req.Text != nil {
...
func responsesContentPartsToChatContent(rawParts []json.RawMessage, role string) (json.RawMessage, error) {
	...
	if !hasNonText {
		// 保留下游桥的既有语义：单一文本折叠为字符串，多段文本仍保留
		// Chat Completions content parts，避免 Anthropic 直连与双重转换结果不一致。
		if len(chatParts) > 1 {
